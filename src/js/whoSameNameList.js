var admittedList = (function () {
    ...
    function _setData() {
        ...
        Student.getSameNameStudentList() // 取得 student 列表
            .then((res) => {
    ...
    async function _handleEditStudentInfo() { // 學生列表 Modal 觸發
        ...
        try {
            const response = await Student.getSameNameStudentInfo(_currentUserId);
            if (!response.ok) { throw response; }

            Student.getSameNameStudentInfo(_currentUserId)
                .then((res) => {
                    return res.json();
                }).then((json) => {
